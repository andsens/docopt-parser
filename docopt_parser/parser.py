...
def atom(excludes):
  ...
  def p():
    ...
    a = yield (
      group ^ Optional.opt ^ OptionsShortcut.shortcut ^ Long.long(excl)
...
class DocoptAst(AstNode):
  ...
  def __str__(self):
    return f'''<Docopt>
  usage:
  {self.indent(self.indent(self.usage))}
  options:
  {self.indent(self.indent(self.options))}'''
