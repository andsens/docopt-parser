from docopt_parser.ast import Argument, Command, DocoptAst, Choice, Long, \
  Multiple, OptionLine, Optional, OptionsShortcut, Sequence, Short, Shorts
from docopt_parser.parser_utils import exclude, flatten_tuples, join_string, splat
import re
from parsec import eof, generate, many, many1, none_of, one_of, optional, regex, sepBy, string
...
def short(excludes):
  return (string('-') >> symbol_char(excludes | one_of('=-')) + optional((space | string('=')) >> arg)) \
    .desc('short option (-a)').parsecmap(splat(Short))

def shorts(excludes):
  return (string('-') >> many1(symbol_char(excludes | one_of('=-'))) + optional((space | string('=')) >> arg)) \
...
def next_option_ident(prev):
  return optional(one_of(' ,') >> (
    short(one_of(' \n')) if isinstance(prev, Long) else long(one_of(' \n')))
  ).parsecmap(lambda o: [prev] + ([o] if o is not None else []))
option_opts = (short(one_of(' \n')) ^ long(one_of(' \n'))).bind(next_option_ident).desc('option identifier')
option_line_terminator = (nl + nl) ^ (nl + eof()) ^ (nl + indent + string('-'))
option_doc = many1(
  exclude(any, option_default ^ option_line_terminator)
).desc('option documentation').parsecmap(join_string)
option_desc = optional(option_doc) + optional(option_default) + optional(option_doc)

option_line = (
  option_opts + optional(space + many1(space) >> option_desc)
).desc('option line').parsecmap(flatten_tuples(splat(OptionLine)))

option_lines = sepBy(option_line, nl + indent)
