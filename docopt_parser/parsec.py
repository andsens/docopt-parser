...
# Copy of https://github.com/sighingnow/parsec.py
# Ran into this issue: https://github.com/sighingnow/parsec.py/issues/29
# which was been closed because of a likely misunderstanding.
# Re-raised the issue (https://github.com/sighingnow/parsec.py/issues/34)
# For now I simply copied it, license is MIT
...
def separated(p, sep, mint, maxt=None, end=None):
    ...
    def sep_parser(text, index):
        cnt, sep_values, res = 0, Value.success(index, []), None
        values = sep_values
        while cnt < maxt:
            res = p(text, index)
            if res.status:
